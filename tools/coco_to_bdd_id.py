...
def bdd_to_coco(cityscapes_id):
    lookup = {
        ...
        7: 4,  # motorcycle
        8: 10, # traffic light
        9: 13, # traffic sign, (COCO only has stop signs, but we can initialze from there)
        10: 7,  # train
    }
    return lookup[cityscapes_id]

def bdd_to_coco_with_rider(cityscapes_id):
    lookup = {
        0: 0,  # ... background
        1: 1,  # person
        2: 1,  # rider ("person", *rider has human right!*)
        3: 3,  # car
        4: 6,  # bus
        5: 8,  # truck
        6: 2,  # bicycle
