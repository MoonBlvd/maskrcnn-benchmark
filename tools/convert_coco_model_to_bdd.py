...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Convert a COCO pre-trained model for use with BDD')
    parser.add_argument(
...
def convert_coco_blobs_to_bdd_blobs(model_dict):
    for k, v in model_dict['blobs'].items():
        if v.shape[0] == NUM_COCO_CLS or v.shape[0] == 4 * NUM_COCO_CLS:
            ...
            bdd_blob = convert_coco_blob_to_bdds_blob(
                coco_blob, args.convert_func
            )
            print(' -> converted shape {}'.format(bdd_blob.shape))
            model_dict['blobs'][k] = bdd_blob


def convert_coco_blob_to_bdds_blob(coco_blob, convert_func):
    ...
    std = coco_blob.std()
    mean = coco_blob.mean()
    bdd_shape = [NUM_BDD_CLS] + list(coco_blob.shape[1:])
    bdd_blob = (np.random.randn(*bdd_shape) * std + mean).astype(np.float32)

    # Replace random parameters with COCO parameters if class mapping exists
    for i in range(NUM_BDD_CLS):
        coco_cls_id = getattr(bdd, convert_func)(i)
        if coco_cls_id >= 0:  # otherwise ignore (rand init)
            bdd_blob[i] = coco_blob[coco_cls_id]

    bdd_shape = [NUM_BDD_CLS * leading_factor] + tail_shape
    return bdd_blob.reshape(bdd_shape)
...
def load_and_convert_coco_model(args):
    ...
    convert_coco_blobs_to_bdd_blobs(model_dict)
    return model_dict
